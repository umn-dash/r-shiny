...
```{r step 2 of cell selection, eval = F}
##This code should **replace** the renderTable({}) call contained within your server.R file!
...
```{r step 3 of cell selection, eval = F}
...
```{r step 4 of cell selection, eval = F}
...
```{r step 5 of cell selection, eval = F}
##This code should **replace** the observeEvent({},{}) we introduced in the previous example!
...
```{r step 6 of cell selection, eval = F}
##This code should **replace** the observeEvent({},{}) we introduced in the previous example!
...
```{r, massaging global file, eval = F}
...
world = read_sf("files/TM_WORLD_BORDERS_SIMPL-0.3.shp") #<--THIS FILE CAN BE DOWNLOADED VIA <https://drive.google.com/file/d/1Ta0GSnoJPyEMFjjbpzE3Z8RD0Mq18RL7/view?usp=sharing>. MAKE SURE TO UNZIP IT AND MOVE IT AND ALL ITS COMPANION FILES INTO YOUR R PROJECT FOLDER!
...
```{r server changes for leafletProxy}
##This code should be **swapped for** the renderLeaflet({}) code provided previously, inside of your server function!
...
```{r global change for leafletProxy, eval=F}
